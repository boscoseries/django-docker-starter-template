...
class USSDViewsets(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            citizen = req.make_request('post', '/citizen', data={
                                           "action": "check-unique",
                                           "phone": phone
                                       })

            if citizen.get('unique'):
                session_data = cache.get_or_set(session_id, {"user": {"phone": phone}})
                ...
                if (session_data.get('menu') == 'engage_hospital') or (
                        session_data['base'] and (text.split('*')[-1] == "4")):
                    hospital = Hospital(
                        session_id=session_id,
                        session_data=session_data,
                        user_option=text.split('*')[-1],
